...
//$_SESSION['message']="";
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(!$hasError){
			$_SESSION["seller_info"]=$name;
			$query = "insert into seller_info values('$id','$name','$uname','$pass','$email','$num','$add','$gender','$birth')";
			execute($query);							  
		}
	}
		
		function authenticateManager($id,$uname,$pass,$email,$num,$address,$gender,$birth){
		$query = "select * from seller_info where ID='$id'and Name='$name' and Username='$uname' and Password='$pass' and Email='$email' and Phone_Number='$num' and Address='$address' and Gender='$gender' and Date_Of_Birth='$birth' ";
		$result = get($query);
		...
		$query = "select * from seller_info where Username='$username'";
		$result = get($query);
