...
//Update User Function
    function updateUser($c_id, $c_name,$c_uname,$c_email,$c_phone, $c_pass)
    {
        $query = "update deliveryman_info set name='$c_name', uname='$c_uname', pass ='$c_pass', email='$c_email', phone='$c_phone'  where id='$c_id'";
	...
	if(empty($_POST["pass"])){
      ...
      if(empty($err_number) && empty($err_email) && empty($err_uname) && empty($err_name) && empty($err_pass)  )
        {
            
            if(isset($_POST["updateDetails"]))
            {
                $result = updateUser($id,$name,$uname,$email,$number,$pass);
